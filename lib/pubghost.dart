...
Future<bool> checkUnusedIntlKeys() async {
  ...
  String? jsonPath;
  String? jsonTranslationAccessor;

  if (pubspecFile.existsSync()) {
    final yaml = loadYaml(pubspecFile.readAsStringSync());
    jsonPath = yaml[packageName]?['json_intl_path'] as String?;
    jsonTranslationAccessor = yaml[packageName]?['translation_accessor'] as String?;
  }
  ...
  for (final key in allKeys) {
    ...
    final patterns = [
      RegExp('["\']$escapedKey["\']\\.tr|["\']$escapedKey["\']\\.$jsonTranslationAccessor'),
      RegExp('\\.$escapedKey(\\b|\\()'),
